...
The one structural difference from the JS version: loadModel there takes an
onProgress *callback*; the generated Python stub is pull-based (an iterator
mixing progress events with the terminal reply) since there's no ergonomic
loadModel() wrapper yet (see qvac.api's module docstring) -- everything else
...
with QvacWorker() as worker:
    ...
    try:
        ...
        model_id = None
        for event in load_model_with_progress(transport, load_request):
        ...
        for chunk in completion_stream(transport, completion_request):
