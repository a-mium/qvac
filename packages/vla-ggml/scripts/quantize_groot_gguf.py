...
any of these produces silent garbage, not a graceful degrade):
  ...
  1. `v.patch_embd.*` and `v.position_embedding.weight` — read via RAW HOST
...
Usage:
    python quantize_groot_gguf.py --in groot.gguf --profile q8_0
    # -> groot-q8_0.gguf  (~4.5GB, ~2x smaller)
"""
...
# floor). Mixed profiles (Q5_K/Q4_K FFN for mobile) can be added once proven.
def target_type(name: str, ndim: int, ne0: int, profile: str) -> QT | None:
    ...
    # intermediate M4.5/M4.6 gates fail. `q8_vf16` keeps vision at F16 (~+0.4GB)
    # and Q8_0's everything else, restoring every parity gate.
    if profile == "q8_vf16" and (name.startswith("v.") or name.startswith("mm.")):
        return None
...
def main() -> int:
    ...
    ap.add_argument("--profile", default="q8_0")
