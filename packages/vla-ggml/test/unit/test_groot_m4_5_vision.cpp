// M4.5 vision-tower parity: grootBuildVisionGraph on the oracle's flattened patch
// input vs vision_output.0 (merged embeds) + vision_output.1.{0,1,2} (deepstack).
// Fixture: 4 images @ 16×16 patch grid, 2×2 merge → 256 merged tokens; vision
// attention is block-diagonal per image (patches attend only within their image).
...
TEST(GrootM4_5, VisionTowerMatchesPytorch) {
  ...
  const std::vector<int> deepstack_idx = {5, 11, 17};
  vw.deepstack_mergers.resize(3);
  for (size_t i = 0; i < 3; ++i) {
    const std::string b = "v.deepstack." + std::to_string(deepstack_idx[i]);
  ...
  const std::vector<float> exp_merged = act.readF32("vision_output.0");
  ASSERT_EQ(exp_merged.size(), size_t(N_MERGED) * OUT_HIDDEN);
  std::vector<std::vector<float>> exp_ds(3);
  for (int i = 0; i < 3; ++i) {
    exp_ds[i] = act.readF32("vision_output.1." + std::to_string(i));
    ASSERT_EQ(exp_ds[i].size(), size_t(N_MERGED) * OUT_HIDDEN);
  }
  ...
  struct ggml_tensor* wlin = grootBuildPatchEmbedLinear(
      c, vw.patch_embd_w, vw.patch_embd_w1, N_EMBD, /*inChannels=*/3,
      /*temporalPatch=*/2, /*patchSize=*/GRID);
  ...
  struct ggml_tensor* out = grootBuildVisionGraph(
      ...
      vw, N_IMAGES, GRID, GRID, N_EMBD, N_HEAD, HEAD_DIM, MERGE, NUM_POS_EMBD,
      OUT_HIDDEN, EPS, ROPE_FREQ_BASE, deepstack_idx, &deepstack, &blocks);
  ...
  for (auto* b : blocks) ggml_build_forward_expand(gf, b);
  ...
  for (int i = 0; i < 24; ++i) {
    ...
    if (i == 0) EXPECT_GT(bc, 0.9999f) << "block 0";
  }

  // STRICT correctness gate — single-block isolation: feed the oracle's block-19
  // output into one block and compare against block-20's oracle output. This
  // removes cross-layer accumulation, so it measures the per-block math alone.
  // ~1.0 here proves every op (LayerNorm, fused-QKV vision M-RoPE, block-diagonal
  // attention, GELU-tanh FFN) is exact; the tower's end-to-end drift below is
  // therefore pure bf16-reference-noise propagation, not an implementation bug.
  {
    ...
    struct ggml_tensor* x19 = ggml_new_tensor_2d(c, GGML_TYPE_F32, N_EMBD, N_POS);
    std::memcpy(x19->data, in19.data(), in19.size() * sizeof(float));
    struct ggml_tensor* b20 = grootBuildVisionBlockGraph(
        c, x19, positions, mask, vw.blocks[20], N_POS, N_EMBD, N_HEAD, HEAD_DIM,
        EPS, ROPE_FREQ_BASE);
  ...
  // End-to-end tower outputs: correct, but capped by 24 layers of bf16-reference
  // accumulation (see isolation gate above). cos > 0.999 / rel < 6% is the bf16
  // floor here, not the 0.9995/2% used for the shallower action-head milestones.
  const size_t n = exp_merged.size();
  const float* got = static_cast<const float*>(out->data);
  const float cos = cosineSim(got, exp_merged.data(), n);
  const float rel = relMaxDiff(got, exp_merged.data(), n);
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    const float dc = cosineSim(g, exp_ds[i].data(), exp_ds[i].size());
    const float dr = relMaxDiff(g, exp_ds[i].data(), exp_ds[i].size());
