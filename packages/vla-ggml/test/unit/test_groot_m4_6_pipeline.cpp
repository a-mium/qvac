...
//
// This is what GrootModel::infer composes. The two pieces NOT reproduced here are
// oracle-fed as they'll be caller-fed in production: text-token embeds (need
// input_ids, not dumped) and the final Gr00tPolicy unnormalization (consumer-side).
...
TEST(GrootM4_6, EndToEndPipelineMatchesPytorch) {
  ...
  std::vector<std::vector<float>> myDeep(3);
  {
    ...
    struct ggml_tensor* pin = ggml_new_tensor_2d(c, GGML_TYPE_F32, IN_FLAT, N_VPOS);
    ...
    struct ggml_tensor* mask = ggml_new_tensor_2d(c, GGML_TYPE_F32, N_VPOS, N_VPOS);
    ...
    struct ggml_tensor* vout = grootBuildVisionGraph(
        c, pin, wlin, vw.patch_embd_b, vw.position_embd, pos, mask, vw, N_IMAGES,
        GRID, GRID, V_EMBD, V_HEAD, V_HEAD_DIM, MERGE, NUM_POS_EMBD, OUT_HIDDEN,
        V_EPS, V_ROPE_BASE, ds_idx, &deep);
    struct ggml_cgraph* gf = ggml_new_graph_custom(c, 16384, false);
    ggml_build_forward_expand(gf, vout);
    for (auto* d : deep) ggml_build_forward_expand(gf, d);
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      std::memcpy(myDeep[i].data(), deep[i]->data, myDeep[i].size() * sizeof(float));
    }
    ggml_free(c);
  }
  ...
  std::vector<float> myBackbone(size_t(T_TOK) * DIM);
  {
    ...
    for (int t = 0; t < T_TOK; ++t)
      if (vpm[t] > 0.5f) {
        std::memcpy(&embeds[size_t(t) * DIM], &myVision[size_t(img) * OUT_HIDDEN],
                    DIM * sizeof(float));
        ++img;
      }
    ...
    for (int ax = 0; ax < 3; ++ax)
      for (int t = 0; t < T_TOK; ++t)
        pp[ax * T_TOK + t] = int32_t(pos_ids[ax * T_TOK + t]);
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      for (int t = 0; t < T_TOK; ++t)
        if (vpm[t] > 0.5f) {
          std::memcpy(&dp[size_t(t) * DIM], &myDeep[i][size_t(im) * DIM],
                      DIM * sizeof(float));
          ++im;
        }
      ds[i] = d;
    ...
    struct ggml_tensor* out = grootBuildTextDecoderGraph(
        c, inpE, posT, mask, ds, tw, T_LAYERS, T_TOK, T_HEAD, T_HEAD_KV,
        T_HEAD_DIM, T_FFN, T_ROPE_BASE, sec, T_RMS_EPS);
  ...
  std::vector<float> myState(size_t(1) * ADIM);
  {
    ...
    vf.vlln_w = gt(ctx_w, "vlfusion.vlln.weight");
    vf.vlln_b = gt(ctx_w, "vlfusion.vlln.bias");
    vf.blocks.resize(VLF_LAYERS);
    for (int i = 0; i < VLF_LAYERS; ++i) {
      const std::string b = "vlfusion.blk." + std::to_string(i);
      auto& bw = vf.blocks[i];
      bw.norm1_w = gt(ctx_w, b + ".norm1.weight");
      bw.norm1_b = gt(ctx_w, b + ".norm1.bias");
      bw.norm3_w = gt(ctx_w, b + ".norm3.weight");
      bw.norm3_b = gt(ctx_w, b + ".norm3.bias");
      bw.attn_q_w = gt(ctx_w, b + ".attn_q.weight");
    ...
    const GrootLinearWeights se1 = linW(ctx_w, "embodiment.state_encoder.layer1");
    const GrootLinearWeights se2 = linW(ctx_w, "embodiment.state_encoder.layer2");
    ...
    // activation outlier (token 0, dim 1793 ≈ 15296) that dominated the backbone
    // cos, exposing+amplifying the vision tower's ~1% bf16 drift on the small
    // channels. So the intermediate vl gate is looser than the oracle-fed M4.1
    // (0.999999). This drift is immaterial downstream: the Euler action output
    // below — fed THESE vl features — still hits cos 0.99999, which is the gate
    // that actually matters. State features (fed the oracle-normalized state)
    // stay near-exact.
  ...
  // ── Phase 4: Euler loop fed by MY vl + MY state features ───────────────
  {
    ...
    for (int step = 0; step < N_STEPS; ++step) {
      ...
      struct ggml_tensor* stateFeat = ggml_new_tensor_2d(c, GGML_TYPE_F32, ADIM, 1);
      ...
      struct ggml_tensor* imMask = ggml_new_tensor_2d(c, GGML_TYPE_F32, T_TOK, t_tok);
      struct ggml_tensor* txMask = ggml_new_tensor_2d(c, GGML_TYPE_F32, T_TOK, t_tok);
      ...
      struct ggml_tensor* af =
          grootBuildActionEncoderGraph(c, actT, tau, aeW1, aeW2, aeW3, ADIM, N_ACT);
      ...
      struct ggml_tensor* out = grootBuildDitGraph(
          c, sa, temb, vl, imMask, txMask, dw, D_LAYERS, N_DHEADS, D_HEAD_DIM,
          ADIM, DIM, D_FFN, OUT_DIM, ATTEND_N, 1e-5f, nullptr);
      struct ggml_tensor* pred = grootBuildCategoryMlpGraph(c, out, dec1, dec2);
      struct ggml_tensor* vel =
          ggml_cont(c, ggml_view_2d(c, pred, ACT_DIM, N_ACT, pred->nb[1], pred->nb[1]));
      struct ggml_cgraph* gf = ggml_new_graph_custom(c, 8192, false);
      ggml_build_forward_expand(gf, vel);
      ASSERT_EQ(pi05_test::computeGraphCpu(gf), GGML_STATUS_SUCCESS);
      const float* velp = static_cast<const float*>(vel->data);
      for (size_t i = 0; i < actions.size(); ++i) actions[i] += dt * velp[i];
      if (step + 1 < N_STEPS) {
